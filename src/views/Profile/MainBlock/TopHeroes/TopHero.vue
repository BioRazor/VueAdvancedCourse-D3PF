<template lang="pug">
    .hero-portrait-wrapper.mb-5.mb-sm-0.hover-cursor-pointer(@click="goToHero(hero.id)")
        .bg-secondary.d-flex.justify-content-center.p-3.p-sm-0
            div(:class="heroClass")
        .p-2.bg-dark
            h5.text-truncate.m-0.text-center.title-name.font-diablo(:class="{'bg-danger': hero.hardcore}")
            |    {{hero.name}}
            img(v-if="hero.seasonal" src="@/assets/img/leaf.png" wigth="12px")
            .d-flex.justify-content-between.border-top.border-secondary.pt-2.align-items-center.mt-2
                small.elite-kills
                    span.text-monospace {{hero.kills.elites | formatNumber}} Elite Kills
